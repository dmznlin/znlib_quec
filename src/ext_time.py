...
class TimeDelta(object):
    ...
    def __add__(self, other):
        if isinstance(other, type(self)):
            return type(self)(
                days=self.days + other.days, seconds=self.seconds + other.seconds
            )
...
class _Date(object):
    ...
    def __is_day_valid(self):
        if self.__month == 2:
            return (
                1 <= self.__day <= 29
                if is_leap_year(self.__year)
                else 1 <= self.__day <= 28
            )
        else:
            return (
                1 <= self.__month <= 31
                if self.__month in [1, 3, 5, 7, 8, 10, 12]
                else 1 <= self.__month <= 30
            )
...
class DateTime(object):
    ...
    def __str__(self):
        return "{} {}".format(self.date, self.time) + (
            " {}".format(self.tz) if self.tz else ""
        )
    ...
    def __calc_year_and_week_day(self):
        ...
        year = self.year - 1
        self.__weekday = (
            year + year // 4 - year // 100 + year // 400 + self.__yearday
        ) % 7
    ...
    def __sub__(self, other):
        if isinstance(other, TimeDelta):
            time_tuple = UtimeAdapter.get_time_tuple_from_timestamp(
                self.timestamp - other.total_seconds()
            )
